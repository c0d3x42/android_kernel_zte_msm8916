...
//mofify by yujianhua for charger pv ftm recovery mode flash blue screen by qualcomm patch
//	ret = mdss_mdp_overlay_start(mfd);
...
//mofify by yujianhua for charger pv ftm recovery mode flash blue screen by qualcomm patch end
	buf = &pipe->back_buf;
	if (mdata->mdss_util->iommu_attached()) {
		if (!mfd->iova) {

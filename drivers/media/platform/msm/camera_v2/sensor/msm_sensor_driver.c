...
static uint16_t g_rear_camera_id = 0;
...
static int32_t UpdateAWBLSC(struct msm_eeprom_ctrl_t *e_ctrl)
{
    ...
    imx214_rg=data[7]*256+data[6];
    imx214_bg=data[9]*256+data[8];
    imx214_golden_rg=data[13]*256+data[12];
    imx214_golden_bg=data[15]*256+data[14];
    //   check_eeprom_id(e_ctrl);
    if(imx214_golden_rg==0)
    {
        printk("this is golden ,return...\n");
        imx214_golden_rg=528;
        imx214_golden_bg=600;
    }
   ...
   imx214_dig_gb=imx214_bg*256/imx214_golden_bg;
	...
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(		      
		s_ctrl->sensor_i2c_client,0x4500, 0x1f, MSM_CAMERA_I2C_BYTE_DATA);		
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(		      
		s_ctrl->sensor_i2c_client,0x0700, 0x01, MSM_CAMERA_I2C_BYTE_DATA);		
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(		      
		s_ctrl->sensor_i2c_client,0x3a63, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
    return rc;
}
#ifdef CONFIG_COMPAT
	...
	void __user *arg)
{
	...
	rc = copy_to_user(ptr_dest, e_ctrl->memory_data,cdata32->cfg.read_data.num_bytes);
...
{
    {0x50, 0x0,  6,   MSM_CAMERA_I2C_BYTE_ADDR, MSM_CAMERA_I2C_BYTE_DATA, 0},	
    {0x50, 0x6,  12,  MSM_CAMERA_I2C_BYTE_ADDR, MSM_CAMERA_I2C_BYTE_DATA, 0},
    {0x50, 0x12, 6,   MSM_CAMERA_I2C_BYTE_ADDR, MSM_CAMERA_I2C_BYTE_DATA, 0},
    ...
    {0x56, 0x0,  86,  MSM_CAMERA_I2C_BYTE_ADDR, MSM_CAMERA_I2C_BYTE_DATA, 0},
};
...
static int32_t read_eeprom_memory(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	msm_sensorinfo_set_back_sensor_module_id(e_ctrl->memory_data[0]);
	pr_err("%s: module id:%d\n", __func__,e_ctrl->memory_data[0]);	

	//
	e_ctrl->memory_data[4] = (uint8_t)(g_rear_camera_id>>8);
	e_ctrl->memory_data[5] = (uint8_t)(g_rear_camera_id&0xff);
	pr_err("%s sensor id %d %d\n", __func__, e_ctrl->memory_data[4],e_ctrl->memory_data[5]);	
	//
    CDBG("%s:X\n",__func__);
    return rc;
}
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	*  sensor control structure */
    ...
    {
        g_rear_camera_id = slave_info->sensor_id_info.sensor_id;
